                                    ...
                                    VO_VIDEO_LAYER_ATTR_S* pstVideoLayerAttr)
{
    ...
    pstVideoLayerAttr->enPixFormat = PIXEL_FORMAT_YUV_SEMIPLANAR_422;
...
extern int ProcessHiFbTde();
int ProcessHiFbTde_PIC();
extern int AVD_ProcessHiFbTde();
extern int AVD_ProcessHiFbTde_PIC();
void thread_create(void)
{
     ...
     if((temp = pthread_create(&thread[1], NULL, PEA_ALGO_PROCESS, NULL)) != 0)  //comment3
         printf("线程2创建失败");
          ...
          printf("线程2被创建\n");
    if((temp = pthread_create(&thread[2], NULL, AVD_ALGO_PROCESS, NULL)) != 0)  //comment3
         printf("线程3创建失败");
     else
          printf("线程3被创建\n");
    if((temp = pthread_create(&thread[3], NULL, AVD_ProcessHiFbTde, NULL)) != 0)  //comment3
         printf("线程3创建失败");
     else
          printf("线程3被创建\n");
    if((temp = pthread_create(&thread[4], NULL, AVD_ProcessHiFbTde_PIC, NULL)) != 0)  //comment3
         printf("线程3创建失败");
     else
          printf("线程3被创建\n");
}
