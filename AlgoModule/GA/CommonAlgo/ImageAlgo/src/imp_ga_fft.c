 ...
 * of a single-precision IpComplex or IpDComplex array by means of an
 * FFT algorithm.
 *
 * The result is a IpComplex/IpDComplex array of the same size, returned
 ...
 *
 * These routines return integer value IP_ERROR if an error was detected,
 * IP_NO_ERROR otherwise.
 *
...
IpDComplex *stageBuff;  /* buffer to hold a row or column at a time */
IpComplex 	*bigBuff;    /* a pointer to a float input array */
IpDComplex *bigBuffd;   /* a pointer to a double input array */


/* Allocate space for stageBuff */
int allocateBuffer(int size)
{
  stageBuff = (IpDComplex*) malloc(size*sizeof(IpDComplex));
  if(stageBuff==(IpDComplex*)NULL)
    {
      handle_error("Insufficient storage for fft buffers");
      return(IP_ERROR);
    }
  else return(IP_NO_ERROR);
}


/* Free space for stageBuff */
IMP_VOID freeBuffer(IMP_VOID)
{
  free((char*)stageBuff);
}
...
IMP_VOID R2TX(int nthpo, IpDComplex *c0, IpDComplex *c1)
{
  int k,kk;
...
IMP_VOID R4TX(int nthpo, IpDComplex *c0, IpDComplex *c1,
          IpDComplex *c2, IpDComplex *c3)
{
  int k,kk;
...
IMP_VOID R8TX(int nxtlt, int nthpo, int length,
          IpDComplex *cc0, IpDComplex *cc1, IpDComplex *cc2, IpDComplex *cc3,
          IpDComplex *cc4, IpDComplex *cc5, IpDComplex *cc6, IpDComplex *cc7)
{
  int j,k,kk;
 ...
 * This routine replaces the input IpDComplex vector by its
 * finite discrete complex fourier transform if in==FFT_FORWARD.
 * It replaces the input IpDComplex vector by its finite discrete
 ...
 */
IMP_VOID FFT842(int direction, int n, IpDComplex *b)
 ...
 */
int fft2f(IpComplex *array, int rows, int cols, int direction)
{
  ...
  if(errflag != IP_NO_ERROR) return(errflag);
 ...
 * inverse if direction==FFT_INVERSE.
 */
int fft2d(IpDComplex *array, int rows, int cols, int direction)
{
  int i, maxsize, errflag;
...
/* Perform forward 2D transform on a IpComplex array. */
int ipForwardFFT2f(IpComplex *array, int rows, int cols)
{
	return(fft2f(array, rows, cols, FFT_FORWARD));
}

/* Perform inverse 2D transform on a IpComplex array. */
int ipInverseFFT2f(IpComplex *array, int rows, int cols)
{
	return(fft2f(array, rows, cols, FFT_INVERSE));
}

/* Perform forward 2D transform on a IpDComplex array. */
int ipForwardFFT2d(IpDComplex *array, int rows, int cols)
{
	return(fft2d(array, rows, cols, FFT_FORWARD));
}

/* Perform inverse 2D transform on a IpDComplex array. */
int ipInverseFFT2d(IpDComplex *array, int rows, int cols)
{
...
IMP_S32 ipFromGrayImage( IMP_U8 * src, IpComplex *array, IMP_U16 width, IMP_U16 height )
{
	...
	IpComplex *p;
	// check image size
	if (  !power_of_2(width) | !power_of_2( height ) )
	{
		return -1;
	}
...
IMP_S32 ipToGrayImage( IpComplex * src, IMP_U8 *dst, IMP_U16 width, IMP_U16 height, float scale )
{
	...
	for ( y = 0; y < height; y++ )
	{
		for ( x = 0; x < width; x++, dst ++, p++ )
		{
			*dst = (IMP_U8) max( 0, min( 255, Magnitude(p->re,p->im) * scale * 255 ) );
		}
	}
	return 1;
}
