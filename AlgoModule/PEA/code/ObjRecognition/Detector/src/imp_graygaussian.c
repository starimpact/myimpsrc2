...
IMP_VOID Mutation(IMP_GrayGaussian_S *pstModule);
...
IMP_S32 IMP_ReleaseGrayGaussian(IMP_MODULE_HANDLE hModule)
{
	...
	pstModule = (IMP_GrayGaussian_S*)hModule;
	GA_HARDWARE_RS_S *pstHwResource; 
	pstHwResource = pstModule->pstHwResource ; 
	IMP_GrayImageDestroy( &pstModule->stFilter, &pstHwResource->stMemMgr );   //释放内存
	IMP_GrayImageDestroy( &pstModule->pstOutput->stFilter, &pstHwResource->stMemMgr );
...
{
	IMP_S32 s32Time = 0;
	IMP_S32 s32Size = pstModule->s32Size;
	IMP_S32 s32Sub = 0 ;
	IMP_S32 s32row; //所在行位置
	IMP_S32 s32col; //所在列位置
    IMP_S32 s32H = pstModule->pstResult->s32Height ;
	IMP_S32 s32W = pstModule->pstResult->s32Width ;
	IMP_S32 *ps32Count = &pstModule->s32Count;
	IMP_U8 *pu8Filter = pstModule->pstOutput->stFilter.pu8Data;
	IMP_U8 *pu8Filtercop = pstModule->stFilter.pu8Data;
	for(s32row=s32H-1;s32row>=0;s32row--)
	{
		IMP_U8 *ptr_filter = (IMP_U8*)(pu8Filter+s32row*s32W);
		IMP_U8 *ptr_filtercop = (IMP_U8*)(pu8Filtercop+s32row*s32W) ;
		for(s32col=s32W-1;s32col>=0;s32col--)
		{
			s32Sub = ptr_filtercop[s32col]-ptr_filter[s32col];
			if(s32Sub!=0)
				s32Time++;
		}	
	}
	if(s32Time-s32Size>0)
		(pstModule->s32Count)++;
	else 
		pstModule->s32Count = 0;
	if(*ps32Count>2)
