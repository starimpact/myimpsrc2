...
SVECTOR *create_svector(WORDSVM *words,char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
  ...
  vec->words = (WORDSVM *)my_malloc(sizeof(WORDSVM)*(fnum));
  for(i=0;i<fnum;i++) { 
      vec->words[i]=words[i];
  }
	...
	SVECTOR is used */
{
    ...
    sum=(WORDSVM *)my_malloc(sizeof(WORDSVM)*veclength);
    sumi=sum;
    ai=a->words;
    bj=b->words;
    while (ai->wnum && bj->wnum) {
...
MODEL *read_model(char *modelfile)
{
  ...
  words = (WORDSVM *)my_malloc(sizeof(WORDSVM)*(max_words+10));
...
int parse_document(char *line, WORDSVM *words, double *label,
