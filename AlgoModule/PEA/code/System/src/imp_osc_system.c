...
IMP_VOID IMP_OSC_ParseProcessorParaData(PEA_SYSTEM_PARA_S *stPara )
{
...
STATUS_E IMP_OSC_GetDmaReq( IMP_HANDLE hModule, DMA_CHANNLS_S *pIdmachs )
{
	pIdmachs->s32ChNum = 1;
#ifdef _TIDSP
...
STATUS_E IMP_OSC_CheckRule( IMP_HANDLE hModule, RULE_CHECK_S *pRulChk )
{
...
extern IMP_VOID IMP_OSC_RULE_InitParaPea( INNER_PARA_S *pstInnerPara, MEM_MGR_ARRAY_S *pMemMgr );
static IMP_VOID IMP_OSC_ReStartStatus( IMP_HANDLE hModule );

//create PEA_MODULE 
STATUS_E IMP_OSC_Create( IMP_HANDLE hModule, MEM_SET_S *pstMems )
{
	...
	pModule->pstResult->s32ModuleSwitch = 2; //open osc function
...
//release PEA_MODULE
STATUS_E IMP_OSC_Release( IMP_HANDLE hModule )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
...
STATUS_E IMP_OSC_InitIdmaChs( IMP_HANDLE hModule, DMA_CHANNLS_S *pIdmachs )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
...
STATUS_E IMP_OSC_UnpackURPRule( URP_PARA_S *pURPpara, RULE_S *pstRule );




STATUS_E IMP_OSC_ConfigArmPeaParameter( IMP_HANDLE hModule, EXTERNAL_PARA_S *pstExtnalPara ,URP_PARA_S *pstURPPara)
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
 ...
 //       IMP_OSD2AAI(pstURPPara);
		if( IMP_OSC_UnpackURPRule( pstURPPara, pRule )!= IMP_STATUS_OK)
		{
			IMP_RULE_DefaultRule( ps8ParRule, pRule, pMemMgr );
	...
	IMP_OSC_ParseProcessorParaData(pModule->pstSysPara );
...
static STATUS_E IMP_OSC_RULE_ReSample(RULE_S *pstRuleSrc, RULE_S *pstRuleDst);
STATUS_E IMP_OSC_ConfigParameter( IMP_HANDLE hModule, EXTERNAL_PARA_S *pstExtnalPara )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	MEM_MGR_ARRAY_S *pMemMgr = &pModule->stHwRes.stMemMgr;
	INNER_PARA_S *pstInnerPara = pModule->pstInnerPara;
	...
	if( pstExtnalPara->u32Type & IMP_PARA_TYPE_RULE )
	{
		if( pRule->stZones.stImage.s32W == IMP_QCIF_IMG_WIDTH && pRule->stZones.stImage.s32H == IMP_QCIF_PAL_IMG_HEIGHT && pstExtnalPara->stRule.stVanaData.s32DatLen >= 41640 )
		{
            ...
            IMP_OSC_RULE_ReSample(pstTmpRule,pRule);
#if defined(IMP_DEBUG_PRINT)
...
IMP_S32 IMP_OSC_DownSampleImage( GRAY_IMAGE_S *pstImageSrc, GRAY_IMAGE_S *pstImageDst )
{
	IMP_S32 x, y, w, h;
	IMP_U8 *dat_p, *dat_i;
...
static STATUS_E IMP_OSC_RULE_ReSample(RULE_S *pstRuleSrc, RULE_S *pstRuleDst)
{
	...
	IMP_OSC_DownSampleImage(&pstRuleSrc->stZones.stImage,&pstRuleDst->stZones.stImage);
	return ret;
}


//transform URP_PARA_S to RULE_S
static STATUS_E IMP_OSC_TansURPToInnerData(URP_PARA_S *pURPpara, RULE_S *pRule)
{
	STATUS_E enRet = IMP_STATUS_OK;
...
//added by mzhang
static STATUS_E IMP_OSC_ReSampleOneRgnRuleDate(URP_POLYGON_REGION_S *pstRgn, IMP_S32 s32SrcImgW, IMP_S32 s32SrcImgH, IMP_S32 s32DstImgW, IMP_S32 s32DstImgH)
{
	STATUS_E enRet = IMP_STATUS_OK;
	int i;
#if PRINT_DBG
...
static STATUS_E IMP_OSC_ReSampleRuleDate(URP_PARA_S *pURPpara, RULE_S *pRule)
{
	STATUS_E enRet = IMP_STATUS_OK;
...
//#ifdef SUPPORT_OSC //bird
		...
		for (dwI = 0; dwI < IMP_MAX_OSC_NUM; dwI++)
		{
			URP_OSC_SPECL_REGIONS_S *psr =  &(pURPpara->stRuleSet.astRule[s32ZoneIndex].stPara.stOscRulePara.astSpclRgs[dwI]);
			IMP_OSC_ReSampleOneRgnRuleDate(&psr->stOscRg, s32SrcImgW, s32SrcImgH, s32DstImgW, s32DstImgH);
			IMP_OSC_ReSampleOneRgnRuleDate(&psr->astSubRgA, s32SrcImgW, s32SrcImgH, s32DstImgW, s32DstImgH);
			IMP_OSC_ReSampleOneRgnRuleDate(&psr->astSubRgB, s32SrcImgW, s32SrcImgH, s32DstImgW, s32DstImgH);
			IMP_OSC_ReSampleOneRgnRuleDate(&psr->astSubRgC, s32SrcImgW, s32SrcImgH, s32DstImgW, s32DstImgH);
		}
//#endif //bird
...
STATUS_E IMP_OSC_UnpackURPRule( URP_PARA_S *pURPpara, RULE_S *pRule )
{
	STATUS_E enRet = IMP_STATUS_OK;
#if PRINT_DBG
printf("Perimeter Boundary Number:%d\n", pURPpara->stRuleSet.astRule[0].stPara.stPerimeterRulePara.stLimitPara.stBoundary.s32BoundaryPtNum);
#endif
	if (IMP_OSC_ReSampleRuleDate(pURPpara,pRule) != IMP_STATUS_OK)
	{
		enRet = IMP_STATUS_FALSE;
		return enRet;
	}
	if (IMP_OSC_TansURPToInnerData(pURPpara,pRule)!= IMP_STATUS_OK)
	{
		enRet = IMP_STATUS_FALSE;
		return enRet;
	}
...
static STATUS_E IMP_OSC_URP_ConfigParameter( IMP_HANDLE hModule, URP_PARA_S *pstURPPara )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	MEM_MGR_ARRAY_S *pMemMgr = &pModule->stHwRes.stMemMgr;
	INNER_PARA_S *pstInnerPara = pModule->pstInnerPara;
...
STATUS_E IMP_OSC_GetParameter( IMP_HANDLE hModule, INNER_PARA_S **ppstInnerPara )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	INNER_PARA_S *pstInnerPara = pModule->pstInnerPara;
...
STATUS_E IMP_OSC_Start( IMP_HANDLE hModule )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
...
//stopping PEA_MODULE
STATUS_E IMP_OSC_Stop( IMP_HANDLE hModule, RESULT_S *pstResult )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	PEA_RESULT_S *pResult = pModule->pstResult;
	PEA_STATUS_S *pStatus = &pResult->stSysStatus;
...
STATUS_E IMP_OSC_GetEnacode( IMP_HANDLE hModule, IMP_U32 *pu32EnaCode )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	INNER_PARA_S *pstInnerPara = pModule->pstInnerPara;
...
static IMP_S32 IMP_OSC_CheckRuleEnable( PEA_MODULE *pModule );
static IMP_S32 IMP_OSC_CheckCurrentImage( PEA_MODULE *pModule );
static IMP_VOID IMP_OSC_OutputTargets( PEA_MODULE *pModule );
...
STATUS_E IMP_OSC_GetTargets( IMP_HANDLE hModule, TGT_SET_S *pstTargetSet )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	PEA_RESULT_S *pResult = pModule->pstResult;
...
STATUS_E IMP_OSC_GetEvents( IMP_HANDLE hModule, EVT_SET_S *pstEvtSet )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	PEA_RESULT_S *pResult = pModule->pstResult;
...
STATUS_E IMP_OSC_GetCommands( IMP_HANDLE hModule, CMD_SET_S *pstCmdSet )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	PEA_RESULT_S *pResult = pModule->pstResult;
...
STATUS_E IMP_OSC_GetResults( IMP_HANDLE hModule, RESULT_S *pstResult )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	PEA_RESULT_S *pResult = pModule->pstResult;
...
STATUS_E IMP_OSC_DoSpecial( IMP_HANDLE hModule, PEA_OPER_S *pstOper )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	STATUS_E enStatus = IMP_STATUS_FALSE;
...
//get rule of hModule
STATUS_E IMP_OSC_GetScene( IMP_HANDLE hModule, IMP_VOID **pRule )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	INNER_PARA_S *pstInnerPara = pModule->pstInnerPara;
...
static STATUS_E IMP_OSC_GetBGImage( IMP_HANDLE hModule, GRAY_IMAGE_S **ppBgImg )
{
	PEA_MODULE *pModule = (PEA_MODULE*)hModule;
	PEA_RESULT_S *pResult = pModule->pstResult;
...
static IMP_VOID IMP_OSC_OutputTargetSet( IpTrackedTargetSet *pstTTargetSet, TGT_SET_S *pstTargetSet,
							IMP_S32 s32OutputAllTgts, IMP_S32 s32OutputPredicted, IMP_S32 s32TrajectDist );
static IMP_VOID IMP_OSC_OutputDregionSet( PEA_DETECTED_REGIONSET_S *pstDrgSet, TGT_SET_S *pstTargetSet, IMP_S32 s32Flag );
static IMP_VOID IMP_OSC_OutputTargetSetOsc( IpTrackedTargetSet *pstTTargetSet, TGT_SET_S *pstTargetSet,
							IMP_S32 s32OutputAllTgts, IMP_S32 s32OutputPredicted, IMP_S32 s32TrajectDist );							
							
#ifdef USE_WATERMARK_DETECOTR

static IMP_VOID IMP_OSC_OutputWaterMark(PEA_MODULE *pPEA,TGT_SET_S *pstTargetSet)
{
	IMP_S32 k;
...
static IMP_VOID IMP_OSC_OutputTargets( PEA_MODULE *pModule )
{
    ...
    {
		IMP_OSC_OutputWaterMark(pModule, &pOutput->stTargetSet);
		return;
    }
#endif
	...
	if( pSysPara->s32OutputDRegion )
	{
		if( (pSysPara->s32OutputDRegion&0x10)==0 )
			IMP_OSC_OutputDregionSet( &pResult->stDRegionSet, &pOutput->stTargetSet, pSysPara->s32OutputDRegion );
