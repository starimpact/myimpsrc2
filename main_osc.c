...
HI_S32 SAMPLE_VIO_TDE_1Screen_VoVGA()
{
   ...
   // SAMPLE_GetViCfg_SD(PIC_CIF, &stViDevAttr, &stViChnAttr);
    ...
    while(1)
    {
        TGT_SET_S	*target_set = &gstOscResult.stTargetSet;
        EVT_SET_S *event_set = &gstOscResult.stEventSet;
        ...
        if(s32TargetNum > 0 )
        {
            ...
            IMP_TDE_DrawPeaResult( s32Handle, &g_stOsdYSurface,&g_stOsdVSurface, &gstOscResult,IMP_D1,IMP_QCIF );

			/** 6. 直接修改YUV数据显示PEA数据结果 绘制轨迹线、用户配置的规则信息等*/
          IMP_DrawOSCResult( &stFrame.stVFrame, &gstOscResult,IMP_D1,IMP_QCIF );
        //  IMP_DrawOSCResult( &stFrame.stVFrame, &gstOscResult,IMP_D1,IMP_CIF );
//printf("Draw!!\n");
...
//#else
 ...
 //     DrawAvdResult( &stFrame.stVFrame, &g_AVD_Reslut);
        ...
        IMP_DrawPeaResult( &stFrame.stVFrame, &gstOscResult,IMP_D1,IMP_CIF );
...
HI_S32 SAMPLE_VIO_TDE_MutiScreen_Vo1280x1024()
{
    ...
    while(1)
    {
        ...
        if(s32TargetNum > 0 )
        {
			...
			/** 5. 通过TDE绘制修改YUV数据显示PEA数据结果 TDE只支持绘制矩形区域、直线等*/
            IMP_TDE_DrawPeaResult( s32Handle, &g_stOsdYSurface,&g_stOsdVSurface, &gstOscResult,IMP_D1,IMP_CIF );
...
void thread_create(void)
{
     ...
     if((temp = pthread_create(&thread[0], NULL, SAMPLE_VIO_TDE_1Screen_VoVGA, NULL)) != 0)       //comment2
         printf("线程1创建失败!\n");
          ...
          printf("线程1被创建\n");
     if((temp = pthread_create(&thread[1], NULL, OSC_ALGO_PROCESS, NULL)) != 0)  //comment3
         printf("线程2创建失败");
     else
          printf("线程2被创建\n");
}
