...
const unsigned char fontdata_sun8x16[FONTDATAMAX] = {
...
void IMP_DrawTextInVbuf(VIDEO_FRAME_S *pVBuf,int x,int y,int scale,char *text,int ylineadd,char *fontlib,int charw,int charh)
{
	char * pVBufVirt_Y;
	char * pVBufVirt_C;
	char * pMemContent;
	...
	while(text[i]!='\0')
	{
		...
		switch(text[i])
		{
			...
			default:
				...
				//printf("1111\n");
				pMemContent=pVBufVirt_Y+liney*pVBuf->u32Stride[0]+linex;
				for(j=0;j<charh;j++)
				{
					pMemContent=pMemContent+scale*pVBuf->u32Stride[0];
					for(k=0;k<charw;k++)
					{
						if(fontlib[offset+j]&(0x80>>k))
						{
							for(m=0;m<scale;m++)
							{
								memset(pMemContent+k*scale+m*pVBuf->u32Stride[0],81,scale);//memset(pMemContent+j*scale*pVBuf->u32Stride[0]+k*scale+m*pVBuf->u32Stride[0],255,scale);
								if((linex+k*scale)%2)
								{
									memset(pVBufVirt_C+linex+k*scale-1+((j*scale+liney+m)/2)*pVBuf->u32Stride[0],240,1);
									memset(pVBufVirt_C+linex+k*scale+((j*scale+liney+m)/2)*pVBuf->u32Stride[0],90,1);
								}
								else
								{
									memset(pVBufVirt_C+linex+k*scale+((j*scale+liney+m)/2)*pVBuf->u32Stride[0],240,1);
									memset(pVBufVirt_C+linex+k*scale+1+((j*scale+liney+m)/2)*pVBuf->u32Stride[0],90,1);
								}
							}
...
#if 0
void DrawAvdResult( VIDEO_FRAME_S *pVBuf, AVD_RESULT_S *pAvdResult)
{
	char strresult[1024];
	memset(strresult,0,1024);
	sprintf(strresult,"Scene:  %4d\nSignal: %4d\nClarity:%4d\nBrightH:%4d\nBrightL:%4d\nColor:  %4d\nFreeze: %4d\n",pAvdResult->stStatclData.s32SceneChg*100,pAvdResult->stStatclData.s32SignalValue,pAvdResult->stStatclData.s32ClarityValue,pAvdResult->stStatclData.s32BrightHValue,pAvdResult->stStatclData.s32BrightLValue,pAvdResult->stStatclData.s32ColorValue,pAvdResult->stStatclData.s32FreezValue);
	//printf("ha\n");
	//printf("Str is %s\n",strresult);
	//printf("xi\n");
	DrawTextInVbuf(pVBuf,8,8,2,strresult,1,fontdata_sun8x16,8,16);
}
#endif
